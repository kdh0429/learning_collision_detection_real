...
if wandb_use == True:
    wandb.init(project="real_CNN_random_Modulize", tensorboard=False)
        ...
        with tf.variable_scope(self.name, reuse = tf.AUTO_REUSE):
            ...
            for i in range(6):
                with tf.variable_scope("Joint"+str(i)+"Net"):
                    for j in range(6):
                        with tf.variable_scope("Data"+str(j)+"Net"):
                            X_input = tf.reshape(self.X[:,num_one_joint_data*i+time_step*j:num_one_joint_data*i+time_step*(j+1)], [-1, 1, time_step, 1])
                            L0 = tf.layers.conv2d(inputs= X_input, filters= 1, kernel_size= [1,3], padding="Valid", activation=tf.nn.relu, kernel_regularizer=tf.contrib.layers.l2_regularizer(regul_factor))
                            L0 = tf.layers.batch_normalization(L0, training=self.is_train)
                            L0 = tf.layers.dropout(L0, rate=1-self.keep_prob, training=self.is_train)
                            L0 = tf.reshape(L0, [-1, time_step-2])
                            if(j == 0):
                                self.L0Concat = L0
                            else:
                                self.L0Concat = tf.concat([self.L0Concat, L0],1)

                    W1 = tf.get_variable("W1", shape=[18, self.hidden_neurons], initializer=tf.contrib.layers.xavier_initializer(), regularizer=tf.contrib.layers.l2_regularizer(regul_factor))
                    b1 = tf.Variable(tf.random_normal([self.hidden_neurons]))
                    L1 = tf.matmul(self.L0Concat, W1) +b1
                    L1 = tf.nn.relu(L1)
...
batch_size = 1000 
total_batch = 705 # joint : 492, random : 705
total_batch_val = 151 # joint: 105, random: 151
total_batch_test = 151 # joint: 105, random: 151
drop_out = 1.0
