        ...
        with tf.variable_scope(self.name, reuse = tf.AUTO_REUSE):
            ...
            for i in range(6):
                with tf.variable_scope("Joint"+str(i)+"Net"):
                    ...
                    W3_1 = tf.get_variable("W3_1", shape=[self.hidden_neurons, self.hidden_neurons], initializer=tf.contrib.layers.xavier_initializer(), regularizer=tf.contrib.layers.l2_regularizer(regul_factor))
                    b3_1 = tf.Variable(tf.random_normal([self.hidden_neurons]))
                    L3_1 = tf.matmul(L3, W3_1) +b3_1
                    tf.identity(L3_1, "Joint"+str(i)+"Net"+"L3_1_matmul")
                    L3_1 = tf.layers.batch_normalization(L3_1, training=self.is_train)
                    tf.identity(L3_1, "Joint"+str(i)+"Net"+"L3_1_batch")
                    L3_1 = tf.nn.relu(L3_1)
                    tf.identity(L3_1, "Joint"+str(i)+"Net"+"L3_1_relu")
                    L3_1 = tf.nn.dropout(L3_1, keep_prob=self.keep_prob)
                    self.hidden_layers += 1

                    W3_2 = tf.get_variable("W3_2", shape=[self.hidden_neurons, self.hidden_neurons], initializer=tf.contrib.layers.xavier_initializer(), regularizer=tf.contrib.layers.l2_regularizer(regul_factor))
                    b3_2 = tf.Variable(tf.random_normal([self.hidden_neurons]))
                    L3_2 = tf.matmul(L3_1, W3_2) +b3_2
                    tf.identity(L3_2, "Joint"+str(i)+"Net"+"L3_2_matmul")
                    L3_2 = tf.layers.batch_normalization(L3_2, training=self.is_train)
                    tf.identity(L3_2, "Joint"+str(i)+"Net"+"L3_2_batch")
                    L3_2 = tf.nn.relu(L3_2)
                    tf.identity(L3_2, "Joint"+str(i)+"Net"+"L3_2_relu")
                    L3_2 = tf.nn.dropout(L3_2, keep_prob=self.keep_prob)
                    self.hidden_layers += 1

                    W3_3 = tf.get_variable("W3_3", shape=[self.hidden_neurons, self.hidden_neurons], initializer=tf.contrib.layers.xavier_initializer(), regularizer=tf.contrib.layers.l2_regularizer(regul_factor))
                    b3_3 = tf.Variable(tf.random_normal([self.hidden_neurons]))
                    L3_3 = tf.matmul(L3_2, W3_3) +b3_3
                    tf.identity(L3_3, "Joint"+str(i)+"Net"+"L3_3_matmul")
                    L3_3 = tf.layers.batch_normalization(L3_3, training=self.is_train)
                    tf.identity(L3_3, "Joint"+str(i)+"Net"+"L3_3_batch")
                    L3_3 = tf.nn.relu(L3_3)
                    tf.identity(L3_3, "Joint"+str(i)+"Net"+"L3_3_relu")
                    L3_3 = tf.nn.dropout(L3_3, keep_prob=self.keep_prob)
                    self.hidden_layers += 1
            ...
            with tf.variable_scope("ConcatenateNet"):
                ...
                tf.identity(L5, "ConcatenateNet"+"L5_batch")
                L5 = tf.nn.relu(L5)
