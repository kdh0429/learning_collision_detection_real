...
for k=num_time_step:size(RawData,1)
    ...
    for i=1:num_time_step
        for j=1:3
            ProcessData(k,num_input*(i-1)+j) = motor_constant1_3*RawData(k-i+1,45+j);
            ProcessData(k,num_input*(i-1)+j+3) = motor_constant4_6*RawData(k-i+1,48+j);
        end
        for j=1:6
            ...
            ProcessData(k,num_input*(i-1)+j+30) = RawData(k-i+1,33+j); % dynamic torque
        end
    end
end
