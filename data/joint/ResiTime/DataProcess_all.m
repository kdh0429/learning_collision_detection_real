...
for k=num_time_step:size(RawData,1)-1
    for i=1:6
        ProcessData(k,num_input*num_time_step+i) = RawData(k+1,15+i);
    end
    ProcessData(k,num_input*num_time_step+7) = RawData(k,58);
    ProcessData(k,num_input*num_time_step+8) = 1-RawData(k,58);
